report 50105 "Calculate Inventory New"
{
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            DataItemTableView = SORTING("Shelf No.") WHERE(Type = CONST(Inventory), Blocked = CONST(false));
            RequestFilterFields = "No.", "Location Filter", "Shelf No.";
    ...
    begin
        ///
        QuantityOnHandBuffer.SetCurrentKey(Line);
        QuantityOnHandBuffer.FindFirst();
        QuantityOnHandBuffer.Reset;
        ...
        if not QuantityOnHandBuffer.FindFirst then begin
            ...
            NewLine += 1000;
            QuantityOnHandBuffer.Line := NewLine;
