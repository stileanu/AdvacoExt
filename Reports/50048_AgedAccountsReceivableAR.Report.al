report 50048 "Aged Accounts Receivable AR"
{
    //Copy of 10040 ICE RSK 1/10/21
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/50048_AgedAccountsReceivableAR.rdl';
            ...
            dataitem(Customer; Customer)
            {
                ...
                dataitem("Cust. Ledger Entry"; "Cust. Ledger Entry")
                {
                    ...
                    begin
                        ...
                        // 05-14-13 ADV Start
                        IF Acct_614_Entry("Cust. Ledger Entry") THEN
                            CurrReport.SKIP;
                        // 05-14-13 ADV End
                    ...
                    begin
                        ...
                        IF "Cust. Ledger Entry"."Document Type" = "Cust. Ledger Entry"."Document Type"::Invoice THEN BEGIN
                            IF SalesInvoiceHeader.GET("Cust. Ledger Entry"."Document No.") THEN BEGIN
                                ...
                                PO := SalesInvoiceHeader."Your Reference";
                            END;
                        END;
    ...
    begin
        ...
        SalCreditMemoLine.RESET;
        SalCreditMemoLine.SETCURRENTKEY("Document No.", "No.");
        IF (PeriodEndingDate[1] > 20121130D) THEN
            InvTrigger := TRUE
        ELSE
            InvTrigger := FALSE;
        InvCustomer := 'TXI-HFAB';
        // 05-14-13 ADV End
    ...
    var
        ...
        SalCreditline: Record "Sales Cr.Memo Line";
        salcreditmemoline: Record "Sales Cr.Memo Line";
        invtrigger: Boolean;
        invcustomer: code[20];
        Orderno: code[20];
        PO: Text[30];
    ...
    local procedure Acct_614_Entry(CustLedgerEntry: Record "Cust. Ledger Entry"): Boolean //ICE RSK 1/10/21
    var
        accounttoeliminate: code[10];
    begin
        accounttoeliminate := '614';
        // 05-14-13 ADV New Function
        CASE CustLedgerEntry."Document Type" OF
            ...
            CustLedgerEntry."Document Type"::Invoice:
                BEGIN
                    ...
                    IF SalesInvoiceLine.FINDFIRST THEN
                        REPEAT
                            IF (SalesInvoiceLine.Type = SalesInvoiceLine.Type::"G/L Account") AND
                               (SalesInvoiceLine."No." = AccountToEliminate) THEN
                                EXIT(TRUE);
            ...
            CustLedgerEntry."Document Type"::"Credit Memo":
                BEGIN
                    SalCreditMemoLine.SETRANGE("Document No.", CustLedgerEntry."Document No.");
                    IF SalCreditMemoLine.FINDFIRST THEN
                        REPEAT
                            IF (SalCreditMemoLine.Type = SalCreditMemoLine.Type::"G/L Account") AND
                               (SalCreditMemoLine."No." = AccountToEliminate) THEN
                                EXIT(TRUE);
                        UNTIL SalCreditMemoLine.NEXT = 0;
                END;
        END;
        EXIT(FALSE);
        // 05-14-13 ADV End
